...
public class ActorSourceGenerator : IIncrementalGenerator
{
    ...
    // GenerateDispatcherMessageContracts method removed - using binary converters instead
        ...
        INamedTypeSymbol classSymbol)
    {
        ...
        foreach (var method in uniqueMethods)
        {
            ...
            // Try to find the corresponding interface method to get BinaryConverter attributes
            IMethodSymbol? interfaceMethod = null;
            if (interfaceSymbol != null)
            {
                interfaceMethod = interfaceSymbol.GetMembers()
                    .OfType<IMethodSymbol>()
                    .FirstOrDefault(m => m.Name == methodName);
            }
            
            // Get BinaryConverter attributes from interface method if available
            var converterAttributes = interfaceMethod?.GetAttributes()
            ...
            // Build parameter deserialization using binary converters with length-prefixing
...
{parameterCode}                    var typedActor = ({fullClassName})actor;");
            ...
            if (hasReturnValue)
            {
                ...
                // Find converter for return value (no ParameterName specified)
                ...
                // Serialize return value using binary converter with length-prefixing
                dispatchCases.AppendLine($@"                    var result = await typedActor.{methodName}({invokeArgsStr});");
                dispatchCases.AppendLine($@"                    using (var ms = new System.IO.MemoryStream())");
                dispatchCases.AppendLine($@"                    {{");
                dispatchCases.AppendLine($@"                        using (var writer = new System.IO.BinaryWriter(ms))");
                dispatchCases.AppendLine($@"                        {{");
                ...
                if (returnConverterAttr != null && returnConverterAttr.ConstructorArguments.Length > 0)
                {
                    ...
                    var converterTypeName = converterType?.ToDisplayString();
                    dispatchCases.AppendLine($@"                            BinaryConverterHelper.WriteWithLength(writer, new {converterTypeName}(), result);");
                }
                else
                {
                    var defaultConverter = GetDefaultConverterForType(innerType);
                    dispatchCases.AppendLine($@"                            BinaryConverterHelper.WriteWithLength(writer, new {defaultConverter}(), result);");
                }
                
                dispatchCases.AppendLine($@"                        }}");
                dispatchCases.AppendLine($@"                        return ms.ToArray();");
                dispatchCases.AppendLine($@"                    }}");
            }
