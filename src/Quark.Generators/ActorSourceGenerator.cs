...
public class ActorSourceGenerator : IIncrementalGenerator
{
        ...
        List<IMethodSymbol> methods)
    {
        ...
        foreach (var method in methods)
        {
            ...
            if (serializableParams.Count > 0)
            {
                ...
                source.AppendLine($"    /// Protobuf message contract for {methodName} request.");
                source.AppendLine($"    /// </summary>");
                source.AppendLine($"    [ProtoContract]");
                source.AppendLine($"    public struct {messageContractPrefix}_{methodName}Request");
                source.AppendLine($"    {{");
            ...
            {
                source.AppendLine($"    /// <summary>");
                source.AppendLine($"    /// Protobuf message contract for {methodName} response.");
                source.AppendLine($"    /// </summary>");
                source.AppendLine($"    [ProtoContract]");
                source.AppendLine($"    public struct {messageContractPrefix}_{methodName}Response");
                source.AppendLine($"    {{");
        ...
        INamedTypeSymbol classSymbol)
    {
        ...
        foreach (var method in uniqueMethods)
        {
            ...
            // Build parameter deserialization using Protobuf
            ...
            {
                // Deserialize Protobuf request message using unique variable name
                parameterCode.AppendLine($"                    {messageContractPrefix}_{methodName}Request protoRequest;");
                parameterCode.AppendLine($"                    using (var ms = new System.IO.MemoryStream(payload))");
                parameterCode.AppendLine($"                    {{");
                parameterCode.AppendLine($"                        protoRequest = ProtoBuf.Serializer.Deserialize<{messageContractPrefix}_{methodName}Request>(ms);");
                parameterCode.AppendLine($"                    }}");
                ...
                foreach (var param in nonCancellationParams)
                {
                    ...
                    parameterCode.AppendLine($"                    var {paramName} = protoRequest.{pascalParamName};");
...
{parameterCode}                    var typedActor = ({fullClassName})actor;");
            ...
            if (hasReturnValue)
            {
                ...
                // Serialize return value using Protobuf
                ...
                dispatchCases.AppendLine($@"                        ProtoBuf.Serializer.Serialize(ms, response);");
