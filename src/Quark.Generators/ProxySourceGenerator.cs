...
public class ProxySourceGenerator : IIncrementalGenerator
{
        ...
        SourceProductionContext context)
    {
        ...
        foreach (var interfaceSymbol in allInterfaceSymbols)
        {
            ...
            // Generate JsonSerializerContext for each interface
            GenerateJsonSerializerContext(context, interfaceName, fullInterfaceName, namespaceName, methods);
        ...
        List<IMethodSymbol> methods)
    {
        ...
        foreach (var method in methods)
        {
            ...
            // Generate request message class if method has parameters
            if (serializableParams.Count > 0)
            {
                source.AppendLine($"    /// <summary>");
                source.AppendLine($"    /// Message contract for {fullInterfaceName}.{methodName} request.");
                source.AppendLine($"    /// </summary>");
                source.AppendLine($"    public sealed class {interfaceName}_{methodName}Request");
                source.AppendLine($"    {{");
            ...
            // Generate response message class if method returns a value
            ...
            {
                source.AppendLine($"    /// <summary>");
                source.AppendLine($"    /// Message contract for {fullInterfaceName}.{methodName} response.");
                source.AppendLine($"    /// </summary>");
                source.AppendLine($"    public sealed class {interfaceName}_{methodName}Response");
                source.AppendLine($"    {{");
    ...
    private static void GenerateProxyMethod(StringBuilder source, string interfaceName, string fullInterfaceName, IMethodSymbol method)
    {
        ...
        if (serializableParams.Count > 0)
        {
            source.AppendLine($"            var jsonRequest = new {interfaceName}_{methodName}Request");
            source.AppendLine($"            {{");
            foreach (var param in serializableParams)
            {
            ...
            source.AppendLine($"            }};");
            source.AppendLine();
            source.AppendLine($"            var payload = JsonSerializer.SerializeToUtf8Bytes(jsonRequest);");
        }
        else
        {
        ...
        if (hasResult)
        {
            ...
            source.AppendLine($"                }}");
            source.AppendLine();
            source.AppendLine($"                var result = JsonSerializer.Deserialize<{interfaceName}_{methodName}Response>(response.ResponsePayload);");
            source.AppendLine($"                if (result == null)");
            source.AppendLine($"                {{");
            source.AppendLine($"                    throw new InvalidOperationException(\"Failed to deserialize response payload.\");");
            source.AppendLine($"                }}");
