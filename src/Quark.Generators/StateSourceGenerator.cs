...
public class StateSourceGenerator : IIncrementalGenerator
{
    ...
    private static bool IsSyntaxTargetForGeneration(SyntaxNode node)
    {
        return node is PropertyDeclarationSyntax propertyDeclaration
            ...
            && propertyDeclaration.Modifiers.Any(m => m.Text == "partial");
    }
        ...
        SourceProductionContext context)
    {
        ...
        // Generate the partial property implementation and Save/Load methods
        var source = $$"""
            // <auto-generated/>
            #nullable enable
            ...
            namespace {{namespaceName}}
            {
                partial class {{className}}
                {
                    ...
                    /// <summary>
                    /// Gets or sets the {{propertyName}} state.
                    /// </summary>
                    public partial {{propertyType}} {{propertyName}}
                    {
                        get => _{{propertyName}} ?? throw new InvalidOperationException($"State '{{propertyName}}' has not been loaded. Call Load{{propertyName}}Async() first.");
                    ...
                    public async Task Load{{propertyName}}Async(CancellationToken cancellationToken = default)
                    {
                        ...
                        _{{propertyName}} = await storage.LoadAsync(ActorId, "{{stateName}}", cancellationToken);
                    ...
                    public async Task Save{{propertyName}}Async(CancellationToken cancellationToken = default)
                    {
                        ...
                        var storage = GetStorage<{{propertyType}}>("{{providerName}}");
                        await storage.SaveAsync(ActorId, "{{stateName}}", _{{propertyName}}, cancellationToken);
                    }
