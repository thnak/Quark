...
## Phase 11 — Collection serialization

- [x] **F-20** Immutable-collection serialization in `[GenerateSerializer]` DTOs — `ImmutableArray<T>`, `ImmutableList<T>`, `ImmutableHashSet<T>`, `ImmutableSortedSet<T>`, `ImmutableDictionary<K,V>`, `ImmutableSortedDictionary<K,V>` supported on both the field-codec path (persistence/streaming) and the static/transport path (grain-call arguments); element/key/value types recursed through existing scalar/enum/GeneratedCodec resolution; unsupported element types produce diagnostic **QRK0054** at codegen time; identity copy in `DeepCopy`; closes #108 — _Complexity: M_
- [x] **F-21** `List<T>` / `Dictionary<K,V>` / `T[]` / `ImmutableStack<T>` / `ImmutableQueue<T>` serialization in `[GenerateSerializer]` DTOs — extends F-20's shape recognition to the mutable BCL collections plus the two remaining immutable sequence shapes; `ImmutableStack<T>` reconstructs via buffer-then-reverse-push to preserve LIFO order (naive sequential `Push` would reverse it); `byte[]` is deliberately excluded from the generic array path (keeps the existing dedicated `ByteArrayCodec`/`GrainMessageSerializer.ByteArray` blob encoding); `DeepCopy`/`CloneStatic` allocate a new container for the mutable kinds (element refs still shared) rather than identity-copying, matching `GrainProxyGenerator`'s `CloneKind.NewList`/`NewArray`/`NewDictionary` convention; nested collection-of-collection members are still rejected via QRK0054 — _Complexity: M_

## Phase 12 — Grain-scope DI extensibility

- [x] **F-22** Opt-in user-service-provider factory (`IGrainUserServiceProviderFactory`, `CompositeServiceProvider`, `AddGrainUserServiceProviderFactory<,>`, `AddQuarkOwnedScoped<T>`) — allows behaviors to inject a satellite dependency resolver per-grain-**type** (not per-activation, not per-call) — shared across every activation of that grain type — eliminating redundant resolution of heavy, reusable user services; replaces the removed `GrainScopeInitializer` family; `BehaviorRegistrationGenerator` auto-emits satellite-provider registration for opted-in behaviors; see [`docs/superpowers/specs/2026-07-10-grain-user-service-provider-factory-design.md`](docs/superpowers/specs/2026-07-10-grain-user-service-provider-factory-design.md); closes #162 — _Complexity: M_
